...
const formatRub = (value: number) => `${new Intl.NumberFormat("ru-RU", { maximumFractionDigits: 0 }).format(Math.round(value))}₽`;
...
export default async function CalendarPage({ searchParams }: CalendarPageProps) {
  ...
  const nextEvent = isCurrentMonth ? events.find((event) => event.date >= now) ?? events[0] ?? null : events[0] ?? null;
      ...
      <div className={styles.container}>
          ...
          <header className={styles.calendarHero}>
            <div className={styles.heroMain}>
              <div className={styles.heroTop}>
                <div>
                  <p className={styles.heroEyebrow}>Платежный календарь</p>
                  <h1 className={styles.title}>{formatMonthTitle(monthStart)}</h1>
                </div>
              ...
              <p className={styles.heroText}>
                Следите за ближайшими списаниями в {monthNamesPrepositional[monthStart.getMonth()]} и быстро переключайтесь
                между днями, где есть платежи.
              </p>

              <div className={styles.heroStats}>
                <span className={styles.heroBadge}>{monthUniqueSubscriptions} активных подписок</span>
                <span className={styles.heroBadgeAccent}>
                  {nextEvent ? `Следующая оплата ${formatShortDayAndMonth(nextEvent.date)}` : "В этом месяце оплат нет"}
                </span>
              </div>
            </div>

            <article className={styles.summaryCard}>
              <p className={styles.summaryLabel}>Всего к оплате за месяц</p>
              <p className={styles.summaryAmount}>{formatRub(monthTotal)}</p>
              <p className={styles.summaryHint}>
                {monthUniqueSubscriptions} {getSubscriptionWord(monthUniqueSubscriptions)} с ожидаемыми списаниями
              </p>
            </article>
          </header>
        ...
        <section className={styles.daySection}>
          <div className={styles.sectionHeader}>
            <div>
              <p className={styles.dayEyebrow}>Выбранный день</p>
              ...
              {selectedDayEvents.map((event) => (
                <article className={styles.eventCard} key={event.id}>
                  ...
                  <div className={styles.eventMain}>
                    ...
                    <p className={styles.eventSubtext}>{event.paymentCardLabel || "Автосписание"}</p>
                  </div>
